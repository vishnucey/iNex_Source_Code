...
#  to pick subject ID based on the intent  -- 20/05    
    ...
    try:
        ...
        list_where_groupby = [item for item in list_where_groupby if item not in calentity]
...
# 20/05           
        ...
        for item in calentity:
            myquery = {"$and": [{"attributeName": str(item)},{"subjectArea": str(subjectid)}, {"timePeriod": str(time_period)},{"maxSummaryLevel": min_summary}]}
            print (myquery)
            mydoc = mycol.find(myquery)
            for y in mydoc:
                pass
        ...
        for item in calentity:
        # 20/05           
            myquery = {"$and": [{"attributeName": str(item)},{"summaryLevel": min_summary},{"timePeriod": str(time_period)}]}
        ...
        samplelist = [item for item in samplelist if item not in calentity]
        print (samplelist)
        try:
            for item in samplelist:
                myquery = {
    ...
    try:
        data_cuts = [item for item in data_cuts if item not in calentity]
        for item in data_cuts:
    ...
    try:
        ...
        for items in calentity:
            myquery = {"$and": [{"attributeName": str(items)}, {"timePeriod": str(time_period)}]}
    ...
    try:
        ...
        try:
            ...
            for key in df.keys():
                ...
                for x in db.attributeMaster.find({"alias": key}):
...
def main():
    a,b = sql_gen({'_permanent': True, 'Intent': 'Agent Performance', 'timeperiod': 'MTD', 'Account Date': ['202004'], 'time_range': ['previous month'], 'combined': None, 'groupby': None, 'vizType': None, 'fileName': None, 'Region': None, 'Line of Business': None, 'Agent': None,'Loss ratio':'Loss ratio','Renewal Policy Count':None,'Section': None})
    print(a,b)
